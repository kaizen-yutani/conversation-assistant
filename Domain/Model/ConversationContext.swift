...
class ConversationContext {
    ...
    func classifySpeaker(text: String, isQuestion: Bool = false) -> Speaker {
        ...
        // If LLM already classified as question, it's the remote speaker
        if isQuestion {
            return .remote
        }
        ...
        // Greetings/transitions - likely remote speaker
        let isRemoteSpeakerPhrase = lowercased.contains("welcome to") ||
        ...
        // Short questions = remote speaker
        if wordCount < 25 && (hasQuestionMark || startsWithQuestion || isFollowUpPhrase || isRemoteSpeakerPhrase) {
            return .remote
        }

        // Long explanations = local speaker
        if wordCount > 30 {
            return .local
        }

        // Medium length with technical terms might be local speaker answering
        if wordCount > 15 {
            return .local
        }
    ...
    func getFullConversation() -> String {
        return historyQueue.sync {
            ...
            return history.map { utterance in
                let role = utterance.speaker == .remote ? "Q" : "A"
    ...
    func getTextForSummarization() -> String {
        ...
        return oldMessages.map { utterance in
            let role = utterance.speaker == .remote ? "User" : "Assistant"
            return "\(role): \(utterance.text)"
        }.joined(separator: "\n")
    }
