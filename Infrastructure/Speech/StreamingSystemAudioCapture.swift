...
class StreamingSystemAudioCapture: NSObject, SCStreamOutput, SCStreamDelegate, @unchecked Sendable {
    ...
    private func setupDeepgramClient(apiKey: String) {
        ...
        deepgramClient?.onConnected = { [weak self] in
            NSLog("✅ StreamingSystemAudio: Deepgram connected and ready")
        ...
        deepgramClient?.onPartialTranscript = { [weak self] text in
            ...
            // Show interim results in UI but don't fire early — let Deepgram endpointing handle utterance boundaries
            DispatchQueue.main.async {
    ...
    func startCapturing() async throws {
        ...
        // Start Deepgram connection early so handshake runs in parallel with ScreenCaptureKit setup
        NSLog("🔊 StreamingSystemAudio: Connecting to Deepgram (early)...")
    ...
    func stream(_ stream: SCStream, didOutputSampleBuffer sampleBuffer: CMSampleBuffer, of type: SCStreamOutputType) {
        ...
        // Always stream audio to Deepgram to keep connection alive
        // Deepgram has its own VAD and will only transcribe when speech is detected
