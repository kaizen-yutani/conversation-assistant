...
/// Matches transcribed text against known technical keywords/topics
/// Returns the best matching topic with associated content
class KeywordMatcher {

    struct Match {
        let keyword: String
        let topic: TechnicalTopic
        let confidence: Double // 0.0 - 1.0
    }

    /// Find the best matching topic from transcribed text
    func findMatch(in text: String) -> Match? {
        ...
        for topic in TechnicalTopic.allCases {
    ...
    func findAllMatches(in text: String, minConfidence: Double = 0.5) -> [Match] {
        ...
        var matches: [Match] = []
        var seenTopics: Set<TechnicalTopic> = []
...
// MARK: - Technical Topics

enum TechnicalTopic: String, CaseIterable {
