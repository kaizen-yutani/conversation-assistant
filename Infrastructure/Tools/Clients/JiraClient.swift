...
class JiraClient: Tool {
    ...
    func getSprintInfo(input: [String: Any]) async throws -> ToolResult {
        ...
        if let idStr = boardIdStr, let id = Int(idStr) {
            ...
            let (boardsUrl, _) = try await getBoardsApiUrl(project: nil, type: nil)
            ...
            if let (issuesData, issuesResponse) = try? await URLSession.shared.data(for: issuesRequest),
               ...
               let issues = try? JSONDecoder().decode(JiraSearchResponse.self, from: issuesData) {
                ...
                if !issues.issues.isEmpty {
                    let total = issues.issues.count
                    let doneStatuses = ["done", "closed", "resolved", "complete", "completed"]
                    let blockedStatuses = ["blocked", "impediment", "on hold"]
                    let inProgressStatuses = ["in progress", "in review", "in development", "code review", "testing"]

                    let done = issues.issues.filter { doneStatuses.contains(($0.fields.status?.name ?? "").lowercased()) }
                    let blocked = issues.issues.filter {
                        let status = ($0.fields.status?.name ?? "").lowercased()
                        let labels = ($0.fields.labels ?? []).map { $0.lowercased() }
                        return blockedStatuses.contains(status) || labels.contains("blocked") || labels.contains("impediment")
                    }
                    let inProgress = issues.issues.filter { inProgressStatuses.contains(($0.fields.status?.name ?? "").lowercased()) }
                    let todo = issues.issues.filter {
                        let status = ($0.fields.status?.name ?? "").lowercased()
                        return !doneStatuses.contains(status) && !blockedStatuses.contains(status) && !inProgressStatuses.contains(status)
                    }

                    let pctDone = total > 0 ? Int(Double(done.count) / Double(total) * 100) : 0
                    content += "### Sprint Health: \(pctDone)% complete (\(done.count)/\(total) done)\n"
                    content += "**In Progress:** \(inProgress.count) | **To Do:** \(todo.count) | **Blocked:** \(blocked.count)\n\n"

                    if !blocked.isEmpty {
                        content += "### \u{1F6D1} Blocked (\(blocked.count)):\n"
                        for issue in blocked {
                            let assignee = issue.fields.assignee?.displayName ?? "Unassigned"
                            let priority = issue.fields.priority?.name ?? ""
                            content += "- **\(issue.key)**: \(issue.fields.summary) [\(priority)] @\(assignee)\n"
                        }
                        content += "\n"
                    }

                    if !inProgress.isEmpty {
                        content += "### In Progress (\(inProgress.count)):\n"
                        for issue in inProgress {
                            let status = issue.fields.status?.name ?? ""
                            let assignee = issue.fields.assignee?.displayName ?? "Unassigned"
                            let issueType = issue.fields.issuetype?.name ?? ""
                            content += "- **\(issue.key)**: \(issue.fields.summary) [\(status)] @\(assignee) (\(issueType))\n"
                        }
                        content += "\n"
                    }

                    if !todo.isEmpty {
                        content += "### To Do (\(todo.count)):\n"
                        for issue in todo {
                            let assignee = issue.fields.assignee?.displayName ?? "Unassigned"
                            let priority = issue.fields.priority?.name ?? ""
                            content += "- **\(issue.key)**: \(issue.fields.summary) [\(priority)] @\(assignee)\n"
                    ...
                    if !done.isEmpty {
                        content += "### Done (\(done.count)):\n"
                        for issue in done.prefix(5) {
                            let assignee = issue.fields.assignee?.displayName ?? "Unassigned"
                            content += "- **\(issue.key)**: \(issue.fields.summary) @\(assignee)\n"
                        }
                        if done.count > 5 {
                            content += "- ... and \(done.count - 5) more completed\n"
                        }
                        content += "\n"
                    }
                }
    ...
    private func getSprintIssuesApiUrl(sprintId: Int) async throws -> (apiUrl: URL, siteUrl: String) {
        if DataSourceConfig.shared.isOAuthAuthenticated(.confluence),
           let token = await OAuthManager.shared.getValidAccessToken(for: .atlassian) {
            ...
            let urlString = "https://api.atlassian.com/ex/jira/\(cloudId)/rest/agile/1.0/sprint/\(sprintId)/issue?maxResults=50&fields=summary,status,assignee,priority,issuetype,labels"
            guard let url = URL(string: urlString) else {
                throw ToolError.executionFailed("Invalid URL")
            }
            return (url, siteUrl)
        ...
        let baseUrl = config.baseUrl.replacingOccurrences(of: "/wiki", with: "")
        let urlString = "\(baseUrl)/rest/agile/1.0/sprint/\(sprintId)/issue?maxResults=50&fields=summary,status,assignee,priority,issuetype,labels"
        guard let url = URL(string: urlString) else {
            throw ToolError.executionFailed("Invalid URL")
        }
        return (url, baseUrl)
