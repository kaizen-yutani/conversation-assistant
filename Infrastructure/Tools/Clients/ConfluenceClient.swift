...
private let confluenceLog = OSLog(subsystem: "com.nikolayprosenikov.conversationassistant", category: "Confluence")
...
class ConfluenceClient: Tool {
    ...
    private func getAuthHeader() async throws -> String {
        ...
        if DataSourceConfig.shared.isOAuthAuthenticated(.confluence) {
            ...
            os_log("OAuth token refresh failed, trying basic auth...", log: confluenceLog, type: .error)
        }
    ...
    func execute(input: [String: Any]) async throws -> ToolResult {
        ...
        do {
            ...
            os_log("search HTTP %{public}d for query: %{public}@", log: confluenceLog, type: .info, httpResponse.statusCode, query)

            guard httpResponse.statusCode == 200 else {
                let errorMessage = parseConfluenceError(statusCode: httpResponse.statusCode, data: data)
                os_log("search error: %{public}@", log: confluenceLog, type: .error, errorMessage)
            ...
            os_log("search: %{public}d results", log: confluenceLog, type: .info, results.results.count)
            ...
            let summary = results.results.map { page in
                let excerpt = page.excerpt ?? "No excerpt available"
                return """
                ## \(page.title)
                Page ID: \(page.id)
            ...
            // Auto-fetch full content of the top result
            var fullContent = ""
            if let topPage = results.results.first {
                os_log("auto-fetching top result: %{public}@ (id: %{public}@)", log: confluenceLog, type: .info, topPage.title, topPage.id)
                let pageResult = try await fetchPageContent(pageId: topPage.id, siteUrl: siteUrl)
                if pageResult.success {
                    fullContent = "\n\n========== FULL PAGE CONTENT ==========\n\n\(pageResult.content)"
                }
            }

            return .success(content: summary + fullContent)
    ...
    private func getApiUrl(cql: String) async throws -> (apiUrl: URL, siteUrl: String) {
        ...
        if DataSourceConfig.shared.isOAuthAuthenticated(.confluence),
           let token = await OAuthManager.shared.getValidAccessToken(for: .atlassian) {
            ...
            if cloudId == nil || siteUrl == nil {
                os_log("cloudId missing, fetching accessible resources...", log: confluenceLog, type: .info)
            ...
            let urlString = "https://api.atlassian.com/ex/confluence/\(cloudId)/wiki/rest/api/content/search?cql=\(encodedCql)&limit=5&expand=excerpt"
        ...
        let urlString = "\(baseUrl)/wiki/rest/api/content/search?cql=\(encodedCql)&limit=5&expand=excerpt"
        guard let url = URL(string: urlString) else {
    ...
    private func fetchAccessibleResources(token: String) async throws {
        ...
        os_log("Found site %{public}@ (cloudId: %{public}@)", log: confluenceLog, type: .info, siteName, cloudId)
    ...
    func getPage(input: [String: Any]) async throws -> ToolResult {
        ...
        if let title = title, pageId == nil {
            ...
            let (searchUrl, siteUrl) = try await getApiUrl(cql: cql)
            ...
            guard let httpResponse = searchResponse as? HTTPURLResponse, httpResponse.statusCode == 200 else {
                let code = (searchResponse as? HTTPURLResponse)?.statusCode ?? 0
                os_log("getPage: title search failed HTTP %{public}d for: %{public}@", log: confluenceLog, type: .error, code, title)
    ...
    private func fetchPageContent(pageId: String, siteUrl: String) async throws -> ToolResult {
        ...
        os_log("fetchPageContent: GET %{public}@", log: confluenceLog, type: .info, apiUrl.absoluteString)

        let (data, response) = try await URLSession.shared.data(for: request)

        guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
            let code = (response as? HTTPURLResponse)?.statusCode ?? 0
            os_log("fetchPageContent: HTTP %{public}d", log: confluenceLog, type: .error, code)
            let errorMessage = parseConfluenceError(statusCode: code, data: data)
            return .success(content: errorMessage)
        }

        // Parse v2 API response
        guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] else {
            return .success(content: "Could not parse page response")
        }

        let title = json["title"] as? String ?? "Untitled"
        let webui = (json["_links"] as? [String: Any])?["webui"] as? String ?? ""

        // v2 body is at body.storage.value
        ...
        os_log("fetchPageContent: OK, title=%{public}@, content length=%{public}d", log: confluenceLog, type: .info, title, content.count)

        return .success(content: """
            # \(title)

            URL: \(siteUrl)/wiki\(webui)
    ...
    private func getPageApiUrl(pageId: String) async throws -> (apiUrl: URL, siteUrl: String) {
        if DataSourceConfig.shared.isOAuthAuthenticated(.confluence),
           let token = await OAuthManager.shared.getValidAccessToken(for: .atlassian) {
            ...
            // Use v2 API — v1 /rest/api/content/{id} returns 410 (Gone)
            let urlString = "https://api.atlassian.com/ex/confluence/\(cloudId)/wiki/api/v2/pages/\(pageId)?body-format=storage"
            guard let url = URL(string: urlString) else {
                throw ToolError.executionFailed("Invalid URL")
            }
            return (url, siteUrl)
        ...
        let baseUrl = config.baseUrl.trimmingCharacters(in: CharacterSet(charactersIn: "/"))
        let urlString = "\(baseUrl)/wiki/api/v2/pages/\(pageId)?body-format=storage"
        guard let url = URL(string: urlString) else {
            throw ToolError.executionFailed("Invalid URL")
        }
        return (url, baseUrl)
    ...
    private func parseConfluenceError(statusCode: Int, data: Data) -> String {
        ...
        default:
            os_log("API error %{public}d: %{public}@", log: confluenceLog, type: .error, statusCode, rawBody)
