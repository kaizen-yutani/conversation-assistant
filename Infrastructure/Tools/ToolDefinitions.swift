...
enum AvailableTools {
    ...
    static let getSprintInfo = ToolDefinition(
        ...
        description: """
            Get current sprint health report with issues grouped by status. USE THIS for:
            - "how is the sprint going", "sprint status", "sprint progress", "sprint health"
            - "current sprint", "what's in this sprint", "sprint issues"
            - "anything blocked", "what's left in the sprint"
            Returns: sprint dates, goal, completion %, blocked/in-progress/todo/done counts, and all issues grouped by status.
            """,
        input_schema: .object([
