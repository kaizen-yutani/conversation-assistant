...
@available(macOS 14.0, *)
class ConversationAssistantDelegate: NSObject, NSApplicationDelegate, NSTextViewDelegate, @unchecked Sendable {
    ...
    var userRoleDropdown: NSPopUpButton!
    ...
    // Streaming question lock - prevent concurrent question processing
    var isProcessingStreamingQuestion = false
    // Queue for questions that arrive while already processing
    var pendingQuestionQueue: [String] = []
    ...
    func setupWindow() {
        ...
        // Check if onboarding needed - first check actual permission, not just the flag
        let hasPermission = CGPreflightScreenCaptureAccess()
        if hasPermission {
            // Permission already granted - ensure flag is set and skip onboarding
            UserDefaults.standard.set(true, forKey: "hasCompletedOnboarding")
        }

        if !hasPermission && !PermissionChecker.hasCompletedOnboarding() {
    ...
    func setupUI() {
        ...
        shortcutsLabel.stringValue = "⌘B Hide  •  ⌘1 Context  •  ⌘2 Timeline"
        ...
        // User Role dropdown (in header, right-most)
        userRoleDropdown = NSPopUpButton(frame: NSRect(x: tabBar.frame.width - 155, y: 5, width: 150, height: 28), pullsDown: false)
        userRoleDropdown.autoresizingMask = [.minXMargin]
        userRoleDropdown.removeAllItems()
        for role in UserRole.allCases {
            userRoleDropdown.addItem(withTitle: role.displayName)
        }
        if let index = UserRole.allCases.firstIndex(of: AppSettings.shared.userRole) {
            userRoleDropdown.selectItem(at: index)
        }
        userRoleDropdown.font = NSFont.systemFont(ofSize: 12, weight: .medium)
        userRoleDropdown.target = self
        userRoleDropdown.action = #selector(userRoleChanged(_:))
        userRoleDropdown.wantsLayer = true
        userRoleDropdown.layer?.cornerRadius = 8
        userRoleDropdown.isBordered = false
        userRoleDropdown.layer?.backgroundColor = NSColor.white.withAlphaComponent(0.1).cgColor
        (userRoleDropdown.cell as? NSPopUpButtonCell)?.arrowPosition = .arrowAtBottom
        tabBar.addSubview(userRoleDropdown)

        // Language dropdown (in header, left of role dropdown)
        languageDropdown = NSPopUpButton(frame: NSRect(x: tabBar.frame.width - 265, y: 5, width: 105, height: 28), pullsDown: false)
...
Add background information here to help the AI give better answers.
...
"""
        ...
        promptInputField = NSTextField(frame: NSRect(x: inputLeftPadding, y: (promptHeight - 24) / 2 - 2, width: inputWidth, height: 24))
    ...
    func showRecordingIndicator() {
        // Recording indicator disabled - recording time now shown in voice bar
    }
    ...
    func scrollFloatingSolution(by delta: CGFloat) {
        guard let scrollView = floatingSolutionScrollView else { return }
    ...
    func performAnalysis(apiKey: String) async {
        ...
        if case .failure(let error) = result {
            ...
            await MainActor.run {
                currentPinnedSolution = finalResponse
                let attributedSolution = formatMessageContent(finalResponse, isQuestion: false)
    ...
    func startListening() {
        ...
        if useStreaming {
            ...
            systemAudioCapture?.onStatusChange = { _ in
                // Status updates disabled - UI shows visual feedback
            }
    ...
    @objc func sendPromptFromInput() {
        ...
        Task {
            ...
            • "How is the sprint"/"sprint status"/"what's blocked" → search_jira with "my tickets in progress"
            • My tickets/work items/bugs/reported → search_jira
            • Docs/process/how-to → search_documentation
            • Code/PRs/commits/who changed → search_codebase (detects "PR", "issue", "bug" keywords)
            • External info → web_search (last resort)

            MULTI-TOOL STRATEGY:
            • Use MULTIPLE tools when helpful (e.g., Jira + GitHub for feature status)
            • Cross-reference: ticket mentions code? search_codebase for PRs matching the ticket key (e.g., "PROJ-123" in branch names)
            • Sprint questions: use search_jira to find in-progress tickets, then optionally search_codebase for related PRs
            • "Is there a ticket for this PR?" → search_jira with the PR title/feature name
            • If first tool returns nothing, try another before giving up

            IMPORTANT: NEVER use get_sprint_info or list_jira_boards tools. For sprint/board questions, ALWAYS use search_jira instead.

            FAILURE RECOVERY:
            • If any tool returns an error or no results → try a DIFFERENT tool or broader query before responding
            • NEVER tell the user a tool failed or ask them for project keys/board IDs — always try alternatives silently

            RESPONSE RULES:
            1. NEVER ask clarifying questions - search with reasonable defaults
            2. NEVER ask for project keys, board IDs, or configuration — figure it out from available tools
            3. ALWAYS include clickable URLs from results
            4. Be concise: 3-5 bullet points, cite sources
            5. When user says "it"/"that", check CONVERSATION SO FAR

            OBFUSCATION: Replace any mention of "Alma Media" or "Almamedia" with "Yutani Corp". Replace Jira project prefixes (e.g., "AM-", "ALMA-", "AJOMARK-") with "YC-" in your response. Keep ticket numbers the same. Replace URLs containing "almamedia" with "yutanicorp" (e.g., "almamedia.atlassian.net" → "yutanicorp.atlassian.net", "github.com/almamedia" → "github.com/yutanicorp"). Apply to ALL URLs containing "almamedia" regardless of domain.
            \(historyContext)
            User role: \(AppSettings.shared.userRole.displayName)
            """
            ...
            let result = await client.chatWithTools(
                ...
                onToolUse: { [weak self] toolNames in
                    DispatchQueue.main.async {
                        ...
                        let toolsStatus = self?.buildToolsStatus(for: toolNames)
                        if let toolsStatus = toolsStatus {
                            self?.updateStreamingStatus(attributed: toolsStatus)
                        }
                    }
                },
    ...
    func processDirectQuestion(_ audioData: Data) {
        ...
        Task {
            ...
            do {
                ...
                // Build user context from OAuth data
                var directUserContext = ""
                let directDisplayName = DataSourceConfig.shared.getValue(for: .confluence, field: "userDisplayName")
                let directEmail = DataSourceConfig.shared.getValue(for: .confluence, field: "userEmail")
                if let name = directDisplayName {
                    directUserContext = "\nCONNECTED USER: \(name)\(directEmail.map { " (\($0))" } ?? "")\n"
                }
                ...
                • Ticket key (PROJ-123) → search_jira (returns full ticket details)
                • "How is the sprint"/"sprint status"/"what's blocked" → search_jira with "my tickets in progress"
                • Ticket/work/bugs/reported → search_jira
                • Docs/process/how-to → search_documentation
                • Code/PRs/commits/who changed → search_codebase
                ...
                MULTI-TOOL STRATEGY:
                • Use MULTIPLE tools when helpful (e.g., Jira + GitHub for feature status)
                • Cross-reference: ticket mentions code? search_codebase for PRs matching the ticket key
                • "Is there a ticket for this PR?" → search_jira with PR title/feature name
                • If first tool returns nothing, try another before giving up

                FAILURE RECOVERY:
                • If any tool fails → try a DIFFERENT tool or broader query silently
                • NEVER tell the user a tool failed or ask for project keys/board IDs

                RESPONSE RULES:
                1. NEVER ask clarifying questions - search with reasonable defaults
                2. NEVER ask for project keys, board IDs, or configuration
                3. ALWAYS include clickable URLs from results
                4. Be concise: 3-5 bullet points, cite sources
                5. If first tool returns nothing useful, try another tool
    ...
    @objc func userRoleChanged(_ sender: NSPopUpButton) {
        let index = sender.indexOfSelectedItem
        guard index >= 0 && index < UserRole.allCases.count else { return }
        AppSettings.shared.userRole = UserRole.allCases[index]
        NSLog("👤 User role changed to: \(AppSettings.shared.userRole.displayName)")
    }
    ...
    @objc func exportSession() {
        ...
        markdown += "**Role:** \(AppSettings.shared.userRole.displayName)\n\n"
        markdown += "**Legend:**\n"
        ...
        Task {
            do {
                ...
                if source == .microphone {
                    let micToolKeywords = ["jira", "ticket", "sprint", "backlog", "issue", "confluence", "documentation", "wiki", "page", "assigned", "my tickets", "my issues", "what's left", "remaining", "blockers", "blocked", "playwright", "github", "pull request", "PR ", " pr ", "commit", "code review", "repository", "repo", "how is the sprint", "sprint going", "sprint progress", "who changed", "last changes"]
                ...
                // SYSTEM AUDIO = REMOTE SPEAKER → Classify and potentially generate answer
                ...
                let toolKeywords = ["jira", "ticket", "sprint", "backlog", "issue", "confluence", "documentation", "wiki", "page", "assigned", "my tickets", "my issues", "what's left", "remaining", "blockers", "blocked", "playwright", "github", "pull request", "PR ", " pr ", "commit", "code review", "repository", "repo", "how is the sprint", "sprint going", "sprint progress", "who changed", "last changes"]
    ...
    func processStreamingTranscript(_ text: String, isFinal: Bool) {
        ...
        // Check cooldown - don't process new questions while still answering
        if let lastAnswer = lastAnswerTime {
            let elapsed = Date().timeIntervalSince(lastAnswer)
            if elapsed < answerCooldown {
                let isClearQuestion = checkForQuestionMarkers(trimmed)
                if !isClearQuestion {
                    NSLog("⏭️ [streaming] COOLDOWN skipped (%.1fs < %.1fs): %@", elapsed, answerCooldown, trimmed)
                    conversationContext.addUtterance(text: trimmed, topic: conversationContext.lastTopic ?? "unknown")
                    return
                }
            }
        ...
        // Prevent concurrent question processing - queue for later
        if isProcessingStreamingQuestion {
            pendingQuestionQueue.append(trimmed)
            NSLog("📋 [streaming] QUEUED (%d pending): %@", pendingQuestionQueue.count, trimmed)
            return
        }
        isProcessingStreamingQuestion = true

        let toolKeywords = ["jira", "ticket", "sprint", "backlog", "issue", "confluence", "documentation", "wiki", "page", "assigned", "blockers", "blocked", "playwright", "github", "pull request", "PR ", " pr ", "commit", "code review", "repository", "repo", "how is the sprint", "sprint going", "sprint progress", "who changed", "last changes"]

        // FAST PATH: If pre-routing matches AND tool keywords match, skip classification entirely
        let trimmedLower = trimmed.lowercased()
        let hasToolKeyword = toolKeywords.contains(where: { trimmedLower.contains($0) })
        if hasToolKeyword && ToolExecutor.shared.hasConfiguredTools && ToolExecutor.shared.preRouteTools(for: trimmed) != nil {
            NSLog("⚡ FAST PATH: skipping classification, going straight to tools for: %@", trimmed)
            let fastStartTime = Date()
            Task {
                let fullText = utteranceBuffer.isEmpty ? trimmed : "\(utteranceBuffer) \(trimmed)"
                utteranceBuffer = ""
                conversationContext.addUtterance(text: fullText, topic: "tools", isQuestion: true)
                await MainActor.run { [self] in
                    showLoading("🔍 Searching...", color: .systemBlue)
                }
                await answerWithToolsForVoice(question: fullText, topic: "tools", messageType: .question, audioSource: .systemAudio)
                let fastMs = Date().timeIntervalSince(fastStartTime) * 1000
                NSLog("⚡ FAST PATH total: %.0fms", fastMs)
                await MainActor.run { [self] in
                    lastAnswerTime = Date()
                    hideLoading()
                    isProcessingStreamingQuestion = false

                    // Drain the queue
                    if let next = pendingQuestionQueue.first {
                        pendingQuestionQueue.removeFirst()
                        NSLog("📋 [streaming] DEQUEUED (%d remaining): %@", pendingQuestionQueue.count, next)
                        processStreamingTranscript(next, isFinal: true)
                    }
                }
            }
            return
        ...
        // NORMAL PATH: classify first, then decide
        Task {
            ...
            await MainActor.run { [self] in
                ...
                // Drain the queue — process next pending question
                if let next = pendingQuestionQueue.first {
                    pendingQuestionQueue.removeFirst()
                    NSLog("📋 [streaming] DEQUEUED (%d remaining): %@", pendingQuestionQueue.count, next)
                    processStreamingTranscript(next, isFinal: true)
    ...
    func streamAnswerWithHaiku(question: String, topic: String, userBackground: String?, messageType: ConversationMessage.MessageType) async {
        ...
        let prompt = """
        You are a real-time conversation assistant for a software engineer.
    ...
    func answerWithTools(question: String, topic: String) async {
        ...
        let displayName = DataSourceConfig.shared.getValue(for: .confluence, field: "userDisplayName")
        let email = DataSourceConfig.shared.getValue(for: .confluence, field: "userEmail")
        let _ = DataSourceConfig.shared.getValue(for: .confluence, field: "userAccountId")
        ...
        • Ticket/work/bugs/reported → search_jira
        • Docs/process/how-to → search_documentation
        • Code/PRs/commits/who changed → search_codebase (detects "PR", "issue" keywords)
        • External/library docs → web_search (last resort)
        ...
        • Ticket key in PRs: search_codebase for PRs matching ticket keys (e.g., "PROJ-123" in branch names)
        • "Is there a ticket for this PR?" → search_jira with PR title/feature name
        ...
        2. NEVER ask clarifying questions or ask for project keys/board IDs
        3. ALWAYS include clickable URLs from every result (Jira, Confluence, GitHub)
        4. Cite sources: "From Jira:", "From Confluence:", "From GitHub:"
        5. If tool returns nothing, try another before saying "not found"
        ...
        // Try local pre-routing to skip Claude's tool-decision round-trip
        let toolStartTime = Date()
        let result: Result<String, Error>
        if let preRouted = ToolExecutor.shared.preRouteTools(for: question) {
            // Pre-fetch tool results immediately
            let toolNames = preRouted.map { $0.name }
            NSLog("⚡ PRE-ROUTED: %@ → %@", question, toolNames.joined(separator: ", "))
            let toolsStatus = buildToolsStatus(for: toolNames)
            await MainActor.run { self.updateStreamingStatus(attributed: toolsStatus) }

            var preResults: [(toolName: String, result: String)] = []
            for tool in preRouted {
                let toolExecStart = Date()
                let toolResult = await ToolExecutor.shared.execute(toolName: tool.name, input: tool.input)
                let toolMs = Date().timeIntervalSince(toolExecStart) * 1000
                let resultPreview = String(toolResult.content.prefix(200))
                NSLog("⚡ PRE-ROUTE tool %@ completed in %.0fms — result: %@", tool.name, toolMs, resultPreview)
                preResults.append((tool.name, toolResult.content))
            }

            let preRouteMs = Date().timeIntervalSince(toolStartTime) * 1000
            NSLog("⚡ PRE-ROUTE total tools: %.0fms, now sending to Claude", preRouteMs)

            result = await client.chatWithPreFetchedResults(
                question: question,
            ...
            let totalMs = Date().timeIntervalSince(toolStartTime) * 1000
            NSLog("⚡ PRE-ROUTE total (tools + Claude): %.0fms", totalMs)
        } else {
            NSLog("⚡ NO PRE-ROUTE match for: %@", question)
            // No confident match — let Claude decide tools
    ...
    func answerWithToolsForVoice(question: String, topic: String, messageType: ConversationMessage.MessageType, audioSource: AudioSource = .systemAudio) async {
        ...
        1. Be concise - 3-5 bullet points max
        2. NEVER ask clarifying questions or ask for project keys/board IDs
        3. ALWAYS include clickable URLs from results (Jira, Confluence, GitHub)
        4. When user says "it"/"that", check conversation history
        5. If first tool returns nothing useful, try another tool
        ...
        // Set up streaming UI - add question first, then streaming answer (latency set on first chunk)
        await MainActor.run { [self] in
            addVoiceMessage(type: .question, content: question, topic: topic, audioSource: audioSource)
            streamingContent = ""
            addStreamingMessage(type: messageType, topic: topic, latencyMs: nil)
        }
        let answerStartQuestionEndTime = questionEndTime  // Capture for first-chunk latency
                ...
                onChunk: { [weak self] (chunk: String) in
                    DispatchQueue.main.async {
                        if !firstChunkReceived {
                            ...
                            if let qEnd = answerStartQuestionEndTime, let self = self, !self.voiceMessages.isEmpty {
                                let latency = Int(Date().timeIntervalSince(qEnd) * 1000)
                                self.voiceMessages[self.voiceMessages.count - 1].responseLatencyMs = latency
                                NSLog("⚡ First token latency: %dms", latency)
                                // Update the latency label in the streaming container
                                if let container = self.currentStreamingContainer,
                                   let label = container.subviews.first(where: { $0.identifier?.rawValue == "streamingLatencyLabel" }) as? NSTextField {
                                    let displayLatency = latency >= 1000 ? String(format: "%.1fs", Double(latency) / 1000.0) : "\(latency)ms"
                                    label.stringValue = "⚡\(displayLatency)"
                                    label.sizeToFit()
    ...
    func addStreamingMessage(type: ConversationMessage.MessageType, topic: String?, latencyMs: Int? = nil) {
        ...
        // Latency label (placeholder — updated on first chunk arrival)
        let latencyLabel = NSTextField(labelWithString: "")
        ...
        latencyLabel.frame = NSRect(x: cardWidth - 160, y: initialHeight - 22, width: 70, height: 16)
    ...
    func updateStreamingStatus(attributed: NSAttributedString) {
        guard let container = currentStreamingContainer else { return }

        if let loadingLabel = container.subviews.first(where: { $0.identifier?.rawValue == "streamingLoadingLabel" }) as? NSTextField {
            loadingLabel.attributedStringValue = attributed
        }
    }

    /// Build an attributed string with SF Symbol icons for tool status display
    func buildToolsStatus(for toolNames: [String]) -> NSAttributedString {
        let result = NSMutableAttributedString()
        let textAttrs: [NSAttributedString.Key: Any] = [
            .font: NSFont.systemFont(ofSize: 12, weight: .medium),
            .foregroundColor: NSColor.white.withAlphaComponent(0.6)
        ]
        result.append(NSAttributedString(string: "Searching ", attributes: textAttrs))
        for (index, name) in toolNames.enumerated() {
            if index > 0 {
                result.append(NSAttributedString(string: ", ", attributes: textAttrs))
            }
            result.append(ToolExecutor.shared.attributedDisplay(for: name, fontSize: 11, color: NSColor.white.withAlphaComponent(0.7)))
        }
        result.append(NSAttributedString(string: "...", attributes: textAttrs))
        return result
    }
